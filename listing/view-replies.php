    ...
    ?>
        ...
        <div id="<?php echo $replyRequestId; ?>" style="display: none;"> 
            <form method="POST" action="includes/create-reply.inc.php">
                ...
                $sql = "SELECT * FROM accounts WHERE id=? AND username=?";
                $stmt = mysqli_stmt_init($connection);

                if(!mysqli_stmt_prepare($stmt, $sql)){
                    echo "images/profile-pic.png";
                } else {    
                    $id = $_SESSION["id"];
                ...
                echo "uploads/profile-pictures/".$row["profile_picture"];
                
                ?>" width="60" alt="Profile Picture" class="rounded-circle m-1"> 
